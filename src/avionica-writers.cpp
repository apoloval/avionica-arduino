...
Max7219Writer::Max7219Writer(Port& port, byte line)
  : port(port), line(line), buffer_index(0), decode_modes(0) {
  port.setupLine(line, AVIONICA_SERIAL_OUTPUT);
  resetBuffer();
}

void Max7219Writer::setNormalOperation() {
  send(MAX7219_SHUTDOWN_ADDR, 1);
}

void Max7219Writer::setDecodeMode(byte digit, byte mode) {
  switch (mode) {
...
void Max7219Writer::setDecodeModeForAll(byte mode) {
  switch (mode) {
    case AVIONICA_MAX7219_NO_DECODE:
      decode_modes = 0x00;
      break;
...
void Max7219Writer::setIntensity(float value) {
...
void Max7219Writer::setScanLimit(byte limit) {
  send(MAX7219_SCAN_LIMIT_ADDR, limit);
}

void Max7219Writer::setNoDecodeDigit(byte digit, byte value) {
  send(MAX7219_DIGIT0_ADDR + digit, value);
}

void Max7219Writer::send(byte addr, byte data) {
...
void Max7219Writer::resetBuffer() {
